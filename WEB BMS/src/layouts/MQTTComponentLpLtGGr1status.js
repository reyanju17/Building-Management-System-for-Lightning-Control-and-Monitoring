...
const MQTTComponentLpLtGGr1status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantaiG_grp1_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantaiG_grp1_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLtGGr1status;
