...
const MQTTComponentLpLt6Gr12status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai6_grp12_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai6_grp12_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt6Gr12status;
