...
const MQTTComponentLpLt2Gr10status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai2_grp10_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai2_grp10_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt2Gr10status;
