...
const MQTTComponentLpLt5Gr4status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai5_grp4_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai5_grp4_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt5Gr4status;
