...
const MQTTComponentLpLt3Gr16status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai3_grp16_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai3_grp16_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt3Gr16status;
