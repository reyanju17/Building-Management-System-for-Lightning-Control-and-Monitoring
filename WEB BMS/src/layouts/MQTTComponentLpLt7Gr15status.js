...
const MQTTComponentLpLt7Gr15status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai7_grp15_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai7_grp15_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt7Gr15status;
