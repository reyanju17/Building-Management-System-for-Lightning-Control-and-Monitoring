...
const MQTTComponentLpLt7Gr7status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai7_grp7_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai7_grp7_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt7Gr7status;
