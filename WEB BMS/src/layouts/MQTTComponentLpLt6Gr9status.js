...
const MQTTComponentLpLt6Gr9status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai6_grp9_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai6_grp9_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt6Gr9status;
