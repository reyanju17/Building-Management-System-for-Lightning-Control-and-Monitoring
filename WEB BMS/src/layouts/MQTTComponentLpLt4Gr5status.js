...
const MQTTComponentLpLt4Gr5status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai4_grp5_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai4_grp5_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt4Gr5status;
