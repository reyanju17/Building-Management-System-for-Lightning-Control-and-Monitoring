...
const MQTTComponentLpLt1Gr12status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai1_grp12_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai1_grp12_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt1Gr12status;
