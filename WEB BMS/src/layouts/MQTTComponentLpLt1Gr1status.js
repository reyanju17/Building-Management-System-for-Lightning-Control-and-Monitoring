...
const MQTTComponentLpLt1Gr1status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai1_grp1_hmi_button",
      qos: 2,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai1_grp1_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt1Gr1status;
