...
const MQTTComponentLpLt4Gr21status = () => {
  ...
  const turnOff = () => {
    const context = {
      topic: "IOT_lantai4_grp21_hmi_button",
      qos: 1,
      payload: '{ "value" : [ false ]}',
    };
    mqttPublish(context);
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("IOT_lantai4_grp21_hmi_button", (err) => {
        if (!err) {
...
export default MQTTComponentLpLt4Gr21status;
